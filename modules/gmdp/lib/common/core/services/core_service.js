 ...
 * @param cb
 */
exports.getOrgRootTreeData = function(org_id,cb) {
    var fields = {_id:1, org_name: 1, org_pid: 1}; // 待返回的字段
    var options = {sort: {'org_pid': 1, 'org_order': 1}};
    userModel.$CommonCoreOrg.find({org_status:1,org_pid:org_id},fields, options, function(error, result) {
        if(error) {
            cb(new Array());
        }
        else {
 ...
 * @param cb
 */
exports.getOrgRootTreeDataAsyn = function(org_id,cb) {
    var fields = {_id:1, org_name: 1, org_pid: 1, child_count:1}; // 待返回的字段
    var options = {sort: {'org_pid': 1, 'org_order': 1}};
    userModel.$CommonCoreOrg.find({org_status:1,org_pid:org_id},fields, options, function(error, result) {
        if(error) {
            cb(new Array());
            ...
            cb(tree.buildEasyuiTreeAsyn(result, '_id', 'org_name', 'org_pid','child_count'));
        }
    });
}
 ...
 * @param cb
 */
exports.getUserData = function(conditionMap,cb) {
    var fields = {_id:1, user_no: 1, user_name: 1}; // 待返回的字段
    var options = {};
    userModel.$.find(conditionMap, fields, options, function(error, result) {
        if(error) {
            cb(new Array());
        }
        else {
                ...
                userModel.$.find({'$or':[{'login_account':account}],user_status:1})
                    ...
                    if(error) {
                     ...
                     if(result.length > 1) {
                         ...
                         cb(utils.returnMsg(false, '1000', '不存在的账号', null, null));
                                    ...
                                    if(userInfo.user_roles.length == 0) {
                                        ...
                                        userInfo.populate(opts, function (err, populatedUserInfo) {
                                            ...
                                            if (populatedUserInfo.user_roles.length == 0) {
                                                ...
                                                populatedUserInfo.user_tag = 1;//(1:内部;2:外部)
                                                // 登陆成功后设置用户所在区域信息

                                                // 获取机构区域信息
                                                var county_id = populatedUserInfo.user_org.org_belong;
                                                if(county_id == "0") {
                                                    populatedUserInfo.user_area.city_id = "0"
                                                    populatedUserInfo.user_area.city_name = "贵州省";
                                                    populatedUserInfo.user_area.county_id = "";
                                                    populatedUserInfo.user_area.county_name = "";
                                                    ...
                                                    memcached_utils.getDict(function(err, dictData){
                                                        var area_info = dictData[config.project.org_belong_mapping_key];
                                                        for(var k = 0; k < area_info.length; k++) {
                                                            if(area_info[k].id == county_id) {

                                                                var city_id = area_info[k].pid;
                                                                if(city_id != '0') {// 区县
                                                                    populatedUserInfo.user_area.county_id = county_id;
                                                                    populatedUserInfo.user_area.county_name = area_info[k].text;
                                                                    populatedUserInfo.user_area.city_id = city_id;
                                                                    for(var l = 0; l < area_info.length; l++) {
                                                                        if (area_info[l].id == city_id) {
                                                                            populatedUserInfo.user_area.city_name = area_info[l].text;
                                                                        }
                                                                    }
                                                                }
                                                                else { // 地市
                                                                    populatedUserInfo.user_area.city_id = county_id;
                                                                    populatedUserInfo.user_area.city_name = area_info[k].text;
                                                                    populatedUserInfo.user_area.county_id = "";
                                                                    populatedUserInfo.user_area.county_name = "";
                                                                }
                                                            }
 ...
 * @param cb
 */
exports.phoneLogin = function(account, password, cb) {

    // 检查账号
    exports.getLoginAccountErrorCount(account, function(count_error, count){
        if(count_error){
            cb(utils.returnMsg(false, '1007', '根据手机号获取用户信息时出现异常。', null, count_error));
        }
        else {
            var password_daily_err_count = config.auth.password_daily_err_count ? config.auth.password_daily_err_count : 5;
            if(count >= password_daily_err_count) {
                ...
                // 根据账号名获取用户信息并关联用户所在机构，所属系统，所拥有权限
                //userModel.$.find({'login_account':account})
                userModel.$.find({'login_account':account,user_status:1})
                    .populate(['user_org', 'user_sys', {path: 'user_roles', match: { 'role_status': { $eq: 1 }}}])
                    .exec(function(error, result){
                        // console.log("查询用户表--》",result);
                        if(error) {
                            ...
                            // 检手机号是否正确
                            if(result.length > 1) {
                                cb(utils.returnMsg(false, '1002', '手机号重复', null, null));
                            }else if(result.length==0){
                                cb(utils.returnMsg(false, '1000', '不存在的手机号', null, null));
                                ...
                                // 检查系统是否禁用
                                if(!userInfo.user_sys) {
                                    cb(utils.returnMsg(false, '1004', '手机号未关联系统', null, null));
                                }
                                else {
                                    if(userInfo.user_roles.length == 0) {
                                        cb(utils.returnMsg(false, '1005', '手机号未设置角色', null, null));
                                    }
                                    else {
                                        var opts = {path: 'user_roles', match: {'sys_id': {$eq: userInfo.user_sys._id}}};
                                        // 过滤掉不是同一系统的角色
                                        userInfo.populate(opts, function (err, populatedUserInfo) {
                                            ...
                                            if (populatedUserInfo.user_roles.length == 0) {
                                                cb(utils.returnMsg(false, '1006', '手机号未设置同一系统下的角色', null, null));
                                            }
                                            else {
                                                populatedUserInfo.user_tag = 1;//(1:内部;2:外部)
                                                // 登陆成功后设置用户所在区域信息
 ...
 * @param cb
 */
exports.saveUserLoginLog = function(user, cb) {
    if(user) {
        var params = [];
        var user_json={};
        user_json.login_account=user.login_account ? user.login_account : '';
        user_json.user_name=user.user_name ? user.user_name : '';
        user_json.user_no=user.user_no ? user.user_no : '';
        user_json.user_phone=user.user_phone ? user.user_phone : '';
        user_json.login_time=new Date();
        params.push(user_json);

        // 保存登陆日志
        userModel.$CommonUserLoginLog.create(params,function(err,res){
                if (err) {
                    cb(utils.returnMsg(false, '1001', '保存登陆日志时出现异常[mysql]', null, null));
                }
                else {
                    cb(utils.returnMsg(true, '0000', '保存登陆日志成功[mysql]', null, null));
                }
        })
    }
    else {
        cb(utils.returnMsg(false, '1000', 'user信息为空[mysql]', null, null));
    }
}
 ...
 * @param cb
 */
exports.getMenusAndOptsByRoles = function(roles, cb) {
    //console.log("role_id : %s", role_id);
 ...
 * @param cb
 */
exports.addShortcutMenu = function(user_id, menu_id, cb) {

    // 检查menu_id是否存在，存在就删除
    roleMenuModel.$CommonShortcutMenu.count({user_id:user_id, menu_id:menu_id}, function(error1, count1) {

        if(count1 > 0) {// 删除
            roleMenuModel.$CommonShortcutMenu.remove({user_id:user_id, menu_id:menu_id}, function (error1) {
                if(error1) {
                    cb(utils.returnMsg(false, '1003', '移除快捷菜单时出现异常', null, error));
                }
                else {
                    cb(utils.returnMsg(true, '0000', '移除快捷菜单成功', null, null));
                }
            });
        }
        else {// 新增
            var menu_shortcut_count = config.datas.menu_shortcut_count ? config.datas.menu_shortcut_count : 12;
            roleMenuModel.$CommonShortcutMenu.count({user_id:user_id}, function(error, count) {
                if(error) {
                    cb(utils.returnMsg(false, '1000', '保存快捷菜单时出现异常', null, error));
                }
                else {
                    if(count >= menu_shortcut_count) {
                        cb(utils.returnMsg(false, '1001', '每个用户最多定义'+menu_shortcut_count+'个快捷菜单', null, null));
                    }
                    else {
                        roleMenuModel.$CommonShortcutMenu({
                            user_id:user_id,
                            menu_id:menu_id
                        }).save(function(error2){
                            if(error2) {
                                cb(utils.returnMsg(false, '1002', '保存快捷菜单时出现异常', null, error2));
                            }
                            else {
                                cb(utils.returnMsg(true, '0000', '保存快捷菜单成功', null, null));
                            }
                        });
                    }
                }
 ...
 * @param cb
 */
exports.getShortcutMenuByUser = function(user_id, cb) {
    roleMenuModel.$CommonShortcutMenu.find({user_id : user_id},{},{sort:{'order_id':1,'_id':1}})
        .populate(['menu_id'])
        .exec(function(error, result) {
            if(error) {
                cb(utils.returnMsg(false, '1000', '获取个人快捷菜单出现异常', null, error));
            }
            else {
                if(result.length == 0) { // 该用户未定义快捷菜单
                    roleMenuModel.$CommonShortcutMenu.find({user_id : '0'},{},{sort:{'order_id':1,'_id':1}})
                        .populate(['menu_id'])
                        .exec(function(error2, result2) {
                            if(error2) {
                                cb(utils.returnMsg(false, '1001', '获取个人快捷菜单出现异常', null, error2));
                            }
                            else {
                                cb(utils.returnMsg(true, '0000', '获取个人快捷菜单成功[默认]', result2, null));
                            }
                        });
                }
                else {
                    cb(utils.returnMsg(true, '0000', '获取个人快捷菜单成功', result, null));
                }
            }
        });
}
