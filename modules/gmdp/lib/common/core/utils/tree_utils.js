...
function buildSysMenuChildTree2(rootkey, key, nodes) {
    var child = new Array();
    ...
    for(var i = 0; i < nodes.length; i++) {
        ...
        if(containArray.join(',').indexOf(nodeItem._id) == -1 && nodeItem.menu_hidden == 0) {
            if(key == nodeItem.menu_pid) {
                var cchild = buildSysMenuChildTree2(rootkey, nodeItem._id, nodes);
                if(cchild.length > 0) {
                    json.children = cchild;
                }
                containArray.push(nodeItem._id);
                json.menu_root_id = rootkey;
                child.push(json);
            }
        }
    }
...
exports.buildSysMenuTree2 = function(nodes) {
    var tree = new Array();
    if(null != nodes) {
        containArray = new Array();
        for(var i = 0; i < nodes.length; i++) {
            ...
            //console.log("json:"+JSON.stringify(json));
            if(containArray.join(',').indexOf(nodeItem._id) == -1 && nodeItem.menu_hidden == 0) {
                var child = buildSysMenuChildTree2(nodeItem._id, nodeItem._id, nodes);
                // 递归增加子节点
                if(child.length > 0) {
                    json.children = child;
                }
                containArray.push(nodeItem._id);
                json.menu_root_id = nodeItem._id;
                tree.push(json);
            }
        }
    }
...
exports.buildSysTree = function(nodes) {
    var tree = exports.buildSysMenuTree2(nodes);
 ...
 * @returns {Array}
 */
exports.buildEasyuiTreeAsyn = function(nodes, id, text, parentid,child_count,attributes) {
    var tree = new Array();
    if(null != nodes) {
        for(var i = 0; i < nodes.length; i++) {
            var nodeItem = nodes[i];
            ...
            returnJson.id = eval('json.' + id) + '';
            returnJson.text = eval('json.' + text);
            var children = eval('json.' + child_count);
            if("0"==children){
                returnJson.state ='open';
            }else{
                returnJson.state ='closed';
            }
