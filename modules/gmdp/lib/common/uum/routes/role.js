...
router.route('/')
    ...
    .post(function(req,res){
        ...
        var role_level = req.body.role_level;//角色级别
        var role_tag = req.body.role_tag;//角色标记
        ...
        if(!role_level) {
            utils.respMsg(res, false, '2006', '角色级别不能为空。', null, null);
        }
        if(!role_tag) {
            utils.respMsg(res, false, '2007', '角色类别不能为空。', null, null);
        }
        ...
        roleEntity.role_level = role_level;
        roleEntity.role_tag = role_tag;
        if(role_remark){
