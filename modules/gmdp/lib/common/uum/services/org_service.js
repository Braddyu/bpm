...
exports.getOrgList = function(page, size, conditionMap, cb) {
    utils.pagingQuery4Eui(model.$CommonCoreOrg, page, size,conditionMap, cb, null, {'org_pid': 1, 'org_type': 1, 'org_order': 1});
};
 ...
 * @param cb
 */
exports.getOrgTree =  function(org_id,cb) {
    var fields = {_id:1, org_name: 1, org_pid: 1, child_count:1}; // 待返回的字段
    var options = {sort: {'org_pid': 1, 'org_order': 1}};
    model.$CommonCoreOrg.find({org_status:1,org_pid:org_id},fields, options, function(error, result) {
        if(error) {
            cb(new Array());
        }
        else {
