...
router.route("/getVerificationcode").post(function (req, res) {
  console.log("获取验证码方法。");
    ...
    userModel.$.find({login_account:phonename}) .exec(function(error, result){
        console.log("查询用户表:",result);
        if(error){
                //utils.returnMsg(false, '1000', '根据账号名获取用户信息时出现异常。', null, error);
                //utils.respJsonData(res, utils.returnMsg(false, '1000', '根据账号名获取用户信息时出现异常', null, error));
            //toLogin(res,"\u624b\u673a\u53f7\u4e0d\u5b58\u5728\uff0c\u83b7\u53d6\u9a8c\u8bc1\u7801\u5931\u8d25",2);
        } else {
            if(result.length==0){
                //cb(utils.returnMsg(false, '1000', '不存在的账号', null, null));
                utils.respJsonData(res, utils.returnMsg(false, '1000', '不存在的账号', null, null));
                //toLogin(res,"\u624b\u673a\u53f7\u4e0d\u5b58\u5728\uff0c\u83b7\u53d6\u9a8c\u8bc1\u7801\u5931\u8d25",2);
            }else {
                var randomNumber = parseInt(((Math.random()*9+1)*1000));
                map.randomNumber = randomNumber;
                console.log("获取验证码1",randomNumber);
            }
...
router.route("/login").get(function (a, c) {
    ...
    if(a.body.flag==1){
        ...
        //utils.respJsonData(c, utils.returnMsg(true, '0000', '验证码发送成功。', '', null));
        var d = a.body.username, b = a.body.password,y=a.body.verificationCode, e = a.body.captcha;
        console.log("输入验证码",y);
        console.log("获取验证码2",map.randomNumber);
        if(map.randomNumber!=y&&y!=''){
            toLogin(c,"\u9a8c\u8bc1\u7801\u9519\u8bef",2);//\u9a8c\u8bc1\u7801\u9519\u8bef
            return;
        }
        if(y==''){
            toLogin(c,"\u9a8c\u8bc1\u7801\u4e0d\u80fd\u4e3a\u7a7a",2);//
