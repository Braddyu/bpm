    ...
    exports.getLoginAccountErrorCount(account, function(count_error, count){
        if(count_error){
            ...
            if(count >= password_daily_err_count) {
                ...
                userModel.$.find({'$or':[{'login_account':account},{'user_phone':account}],user_status:1})
                    ...
                    if(error) {
                        ...
                        if(result.length == 0) {
                            ...
                            // 此处检查mysql，账号不存在后验证mysql中的代维相关人员的信息
                                var sql = "select a.id as _id, a.`name` as user_name, a.user_name as login_account, a.`password` as login_password, a.emp_role as user_roles_code, a.belong_department as org_code, a.cities as city, a.counties as county, b.org_code as company_code, b.org_name as company_name, c.id as org_id, c.org_name as org_name, c.org_pid as org_pid, c.belong_city as org_city_id, c.belong_area as org_county_id from aptitude_user_info a inner join aptitude_org_info b on a.operating_company = b.org_code inner join aptitude_org_info c on a.belong_department = c.org_code where (a.user_name = ? or a.phone = ?) and NOW() BETWEEN b.start_time and b.end_time and a.removeUser = 0 and a.status = 'zzStatus' and a.employee_status = 3";
                                var params = [account,account];
                                mysql_utils.query(sql, params, function(error, result){
                                    if(error) {
                                        cb(utils.returnMsg(false, '1007', '查询mysql时出现异常[mysql]', null, null));
                                    }
                                    else {
                                        if(result.length != 1) {
                                            cb(utils.returnMsg(false, '1001', '账号不存在[mysql]', null, null));
                                        }
                                        else {
                                            // 查找到数据，检查密码是否正确
                                            var mysql_user = result[0];
                                            var mysql_user_result = {};
                                            if(mysql_user.login_password != password) {

                                                // 此处保存错误日志记录
                                                exports.saveUserLoginErrorLog(account, password, function(log_error){
                                                    if(log_error) {
                                                        cb(utils.returnMsg(false, '1009', '根据账号名获取用户信息时出现异常。[mysql]', null, log_error));
                                                    }
                                                    else {
                                                        cb(utils.returnMsg(false, '1003', '密码错误[mysql]', null, null));
                                                    }
                                                });
                                            }
                                            else {
                                                // 获取代维公司人员相关角色
                                                var user_roles_code = mysql_user.user_roles_code;
                                                if(!user_roles_code) {
                                                    cb(utils.returnMsg(false, '1005', '账号未设置角色[mysql]', null, null));
                                                }
                                                else {
                                                    // 角色中的代维人员角色
                                                    //user_roles_code = user_roles_code.replace(new RegExp('D_R_WHY,','gm'),'').replace(new RegExp('D_R_WHY','gm'),'');

                                                    if(!user_roles_code) {
                                                        cb(utils.returnMsg(false, '1010', '代维人员无权登陆PC端[mysql]', null, null));
                                                    }
                                                    else {

                                                        //mysql_user.user_tag = 2;//(1:内部;2:外部)
                                                        mysql_user_result.user_tag = 2;//(1:内部;2:外部)

                                                        mysql_user_result._id = mysql_user._id;
                                                        mysql_user_result.user_name = mysql_user.user_name;
                                                        mysql_user_result.login_account = mysql_user.login_account;
                                                        mysql_user_result._id = mysql_user._id;

                                                        // 分割角色
                                                        var user_roles = user_roles_code.split(',');
                                                        if(user_roles.length == 0) {
                                                            cb(utils.returnMsg(false, '1011', '代维人员无权登陆PC端[mysql]', null, null));
                                                        }
                                                        else {

                                                            // 更新人员是否登录标记
                                                            mysql_utils.query('update aptitude_user_info set is_logged_on = 1 where id = ?', [mysql_user._id], function(uerror, uresult) {
                                                                if(uerror) {
                                                                    cb(utils.returnMsg(false, '1015', '更新代维公司人员状态时出现异常[mysql]', null, null));
                                                                }
                                                                else {

                                                                    console.log('user_roles:' + user_roles_code);
                                                                    // 查询角色信息
                                                                    userModel.$CommonCoreRole.find({'role_code': {'$in': user_roles}}).exec(function (error, result) {
                                                                        if (error) {
                                                                            cb(utils.returnMsg(false, '1012', '查询代维人员角色时出现异常[mysql]', null, null));
                                                                        }
                                                                        else {
                                                                            //mysql_user.user_roles = result;
                                                                            mysql_user_result.user_roles = result;

                                                                            // 获取角色对应的系统
                                                                            if (result.length > 0) {
                                                                                var user_sys_id = result[0].sys_id;
                                                                                // 获取信息信息
                                                                                userModel.$CommonCoreSys.find({'_id': user_sys_id}).exec(function (error, result) {
                                                                                    if (error) {
                                                                                        cb(utils.returnMsg(false, '1013', '查询代维人员所属系统时出现异常[mysql]', null, null));
                                                                                    }
                                                                                    else {
                                                                                        //mysql_user.user_sys = result[0];
                                                                                        mysql_user_result.user_sys = result[0];

                                                                                        // 构建mysql用户机构
                                                                                        mysql_user_result.user_org = {
                                                                                            _id: mysql_user.org_id,
                                                                                            org_code: mysql_user.org_code,
                                                                                            org_name: mysql_user.org_name,
                                                                                            org_pid: mysql_user.org_pid,
                                                                                            org_belong: mysql_user.org_county_id,
                                                                                            org_pcode: mysql_user.company_code,
                                                                                            org_pname: mysql_user.company_name
                                                                                        };
                                                                                        var user_area = {};
                                                                                        memcached_utils.getDict(function (err, dictData) {
                                                                                            ...
                                                                                            for (var k = 0; k < area_info.length; k++) {
                                                                                                if (area_info[k].id == mysql_user.org_county_id) {
                                                                                                    user_area.county_id = area_info[k].id;
                                                                                                    user_area.county_name = area_info[k].text;
                                                                                                }
                                                                                                if (area_info[k].id == mysql_user.org_city_id) {
                                                                                                    user_area.city_id = area_info[k].id;
                                                                                                    user_area.city_name = area_info[k].text;
                                                                                                }
                                                                                            }
                                                                                            mysql_user_result.user_area = user_area;
                                                                                            console.log('mysql:' + JSON.stringify(mysql_user_result));
                                                                                            cb(utils.returnMsg(true, '0000', '登录成功[mysql]', mysql_user_result, null));
                                                                                            exports.saveUserLoginLog(mysql_user_result, function(logResult){
                                                                                ...
                                                                                cb(utils.returnMsg(false, '1014', '未找到账号所属系统[mysql]', null, null));
                                                                            }
                                                                        }
                                                                    });
                                                                }
...
exports.saveUserLoginLog = function(user, cb) {
    if(user) {
        ...
        params.push(utils.getUUID());
        params.push(user.login_account ? user.login_account : '');
        params.push(user._id ? user._id.toString() : '');
        //params.push('');
        params.push(user.user_name ? user.user_name : '');
        params.push(user.user_tag ? user.user_tag : '');
        params.push("pc");
        params.push(new Date());
        params.push(2);
        // 检查机构信息
        if(user.user_org) {
            params.push(user.user_org._id ? user.user_org._id.toString() : '');
            params.push(user.user_org.org_name ? user.user_org.org_name : '');
        }
        else {
            params.push('');
            params.push('');
        ...
        // 保存登陆日志
        mysql_utils.query('insert into common_user_login_log(id,login_account,user_id,name,user_tag,phone_info,create_time,login_port,org_id,org_name) values(?,?,?,?,?,?,?,?,?,?)', params, function(error, result) {
            if (error) {
