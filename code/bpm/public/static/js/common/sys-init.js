...
var InitSys = function() {
    ...
    return {
        ...
        initNavLeftMenu : function (code) {
            var currentOpenMenu = $("ul[class='nav sidebar-menu']>li[class='open']");
            var currentMenu = $("ul[class='nav sidebar-menu'] a[href='"+code+"']");
            if(currentMenu.length > 0) {
                currentOpenMenu.removeClass('open');
                // 选中当前菜单
                currentMenu.parent().addClass('active');
                // 如果当前菜单非第一层
                if(!currentMenu.parent().parent().hasClass('sidebar-menu')){
        ...
        initRevChildMenu:function(id) {
            var currentMenu = $("li[_t_nav='nav_"+id+"']");
            if(currentMenu.length > 0) {
                currentMenu.addClass('nav-up-selected-inpage');
            }
        },
        logout : function (url) {
