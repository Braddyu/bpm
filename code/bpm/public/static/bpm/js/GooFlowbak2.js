...
//初始化区域图的对象
	...
	if(property.haveTool){
     ...
     +"<a href='javascript:void(0)' type='direct' class='GooFlow_tool_btn' id='"+this.$id+"_btn_direct'><i class='ico_direct'/></a><div class='GooFlow_tool_text'>关系</div>"
    );
		if(property.toolBtns&&property.toolBtns.length>0){
			tmp="<span/>";
			for(var i=0;i<property.toolBtns.length;++i){
				tmp+="<a href='javascript:void(0)' type='"+property.toolBtns[i]+"' id='"+this.$id+"_btn_"+property.toolBtns[i].split(" ")[0]+"' class='GooFlow_tool_btn'><i class='ico_"+property.toolBtns[i]+"'/></a><div class='GooFlow_tool_text'>"+ property.toolText[i] +"</div>";//加入自定义按钮
			}
			this.$tool.append(tmp);
		}
		//加入区域划分框工具开关按钮
		if(property.haveGroup)
			this.$tool.append("<span/><a href='javascript:void(0)' type='group' class='GooFlow_tool_btn' id='"+this.$id+"_btn_group'><i class='ico_group'/></a><div class='GooFlow_tool_text'>"+ property.groupText +"</div>");
		toolWidth=31;
		this.$nowType="cursor";
		//绑定各个按钮的点击事件
	...
	if(this.$editable){
	  ...
	  this.$lineMove.on("mousedown",{inthis:this},function(e){
		  ...
		  var lm=$(this);
		  lm.css({"background-color":GooFlow.prototype.color.lineFont||"#333"});
		  var This=e.data.inthis;
...
//          }
	...
	focusItem:function(id,bool){
        ...
        jq.css("border-color",GooFlow.prototype.color.line);
			...
			else	jq[0].strokeColor=GooFlow.prototype.color.mark||"#ff8800";
			if(!this.$editable)	return;
			var x,y,from,to,n;
			if(GooFlow.prototype.useSVG!=""){
