...
function GooFlow(property){
	...
	this.$onNodeFocus = property.onNodeFocus;
	var bgDiv = property.dom;
	...
	// 流程图的名称
	this.$title = "iflow1";
	// 每一种结点或按钮的说明文字,JSON格式,key为类名,value为用户自定义文字说明
	this.$nodeRemark = {};
	// 当前要绘制的对象类型
	this.$nowType = "cursor";
	this.$lineData = {};
	...
	if(property.haveTool){
		this.$bgDiv.append("<div class = 'GooFlow_tool'><div style = 'width:"+ (toolWidth-5) +"px;height:"+(height - 6)+"px;' class = 'GooFlow_tool_div'></div></div>");
		...
		// 连接线
		this.$tool.append("<a href='javascript:void(0)' type='direct' class='GooFlow_tool_btn' id='"+this.$id+"_btn_direct'><b class='ico_direct'/></a><div class='GooFlow_tool_text'>关系</div>");
		this.$tool.append("<span/>");

		if(property.toolBtns && property.toolBtns.length > 0){
			var tools = "";
			for(var i = 0;i<property.toolBtns.length;++i){
				// 工具栏
				tools += "<a href='javascript:void(0)' type = '"+property.toolBtns[i]+"' id = '"+this.$id+"_btn_"+property.toolBtns[i].split(" ")[0]+"' class = 'GooFlow_tool_btn'><b class = 'ico_"+property.toolBtns[i]+"'/></a><div class='GooFlow_tool_text'>"+ property.toolText[i] +"</div>";
			}
			this.$tool.append(tools);
		}
		//加入区域划分框工具开关按钮
		if(property.haveGroup)
			this.$tool.append("<span/><a href = 'javascript:void(0)' type = 'group' class = 'GooFlow_tool_btn' id = '"+this.$id+"_btn_group'><b class = 'ico_group'/></a><div class='GooFlow_tool_text'>"+ property.groupText +"</div>");
		this.$nowType = "cursor";
		//绑定各个按钮的点击事件
		...
		workAreaCss = "border:none;";
	}

	height = height - 2;

	this.$bgDiv.append("<div class = 'GooFlow_work' style = 'width:"+ (width-2) +"px;height:"+ (height-14) +"px;"+ workAreaCss +"'></div>");
	this.$workArea = $("<div class = 'GooFlow_work_inner' style = 'width:"+ width +"px;height:"+ (height-13) +"px'></div>")
...
GooFlow.prototype = {
	...
	initDraw:function(id,width,height){
		if(GooFlow.prototype.useSVG  !=  ""){
			...
			defs.appendChild(GooFlow.prototype.getSvgMarker("arrow2", "#D3D3D3"));
	...
	addNode:function(id,json){
		...
		if(json.type.indexOf(" round") < 0){
			if(!json.width || json.width < this.defaultNodeWidth){
				json.width = this.defaultNodeWidth;
			}
			if(!json.height || json.height < this.defaultNodeHeight){
				json.height = this.defaultNodeHeight;
			}
	...
	focusItem:function(id,bool){
		...
		if(this.$onNodeFocus){
			var type = 'line';
			if(jq.prop("tagName") == "DIV"){
				if(jq[0].childNodes[0]
					&& jq[0].childNodes[0].childNodes[0]
					&& jq[0].childNodes[0].childNodes[0].childNodes[0]
					&& jq[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0]
					&& jq[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0]){
					// className,能够代表组件类型
					type = jq[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].className;
				}
			}
			this.$onNodeFocus(jq, type, id);
		}
	...
	// 更新一个字段信息
	updataItem: function(id, field, value){
		if(!id || !field){
			return;
		}
		var dest = null;
...
///////////以下为有关画线的方法
	...
	drawLine:function(id,sp,ep,mark,dash){
		...
		if(GooFlow.prototype.useSVG != ""){
			...
			if(mark)    line.strokeColor = "#D3D3D3";
	...
	drawPoly:function(id,sp,m1,m2,ep,mark){
		...
		if(GooFlow.prototype.useSVG != ""){
			...
			poly.style.cursor = "pointer";
			if(mark)    poly.strokeColor = "#D3D3D3";
	...
	calcStartEnd:function(n1,n2){
		//X判断：
		var x11 = parseInt(n1.left);
		var x12 = parseInt(n1.left) + parseInt(n1.width);
		var x21 = parseInt(n2.left);
		var x22 = parseInt(n2.left) + parseInt(n2.width);
		...
		//Y判断：
		var y11 = parseInt(n1.top);
		var y12 = parseInt(n1.top) + parseInt(n1.height);
		var y21 = parseInt(n2.top);
		var y22 = parseInt(n2.top) + parseInt(n2.height);
...
jQuery.extend({
	createGooFlow: function(property){
		...
		// 构造流程图
		var designer = new GooFlow(property);
