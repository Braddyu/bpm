...
var utils = require('../app/utils/app_utils');
 ...
 * @param cb
 */
exports.getSysParamFromAll = function(param_name, cb) {
    this.getVal('common_sys_param_data', function(err, data){
        if(param_name) {
            var param_val = null;
            for(var sysid in data) {
                var sysparam = data[sysid];
                for(var key in sysparam) {
                    if(key == param_name) {
                        param_val = sysparam[key];
                    }
                }
            }
            cb(param_val);
