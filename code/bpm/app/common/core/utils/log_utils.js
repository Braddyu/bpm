    ...
    log4js.configure({
        ...
        "appenders": [
            ...
            },
            {
                "category": "info",
                type: "file",
                filename: logPath+"/"+app_id+"_app_info.log",
            ...
            },
            {
                "category": "warn",
                type: "file",
                filename: logPath+"/"+app_id+"_app_warn.log",
                maxLogSize: 10*1024*1024, // = 10Mb
                numBackups: 50, // keep five backup files
                compress: true, // compress the backups
                encoding: 'utf-8',
            ...
            },
            {
                "category": "error",
                type: "file",
                filename: logPath+"/"+app_id+"_app_error.log",
                maxLogSize: 10*1024*1024, // = 10Mb
                numBackups: 50, // keep five backup files
                compress: true, // compress the backups
                encoding: 'utf-8',
            ...
            },
            {
                "category": "debug",
                type: "file",
                filename: logPath+"/"+app_id+"_app_debug.log",
                maxLogSize: 10*1024*1024, // = 10Mb
                numBackups: 50, // keep five backup files
                compress: true, // compress the backups
                encoding: 'utf-8',
...
helper.debug = function(code,info,msg){
    if(msg == null)
        msg = "";
    logDebug.debug({code:code,info:info,msg:msg});
    logConsole.debug({code:code,info:info,msg:msg});
};
helper.info = function(code,info,msg){
    if(msg == null)
        msg = "";
    logInfo.info({code:code,info:info,msg:msg});
    logConsole.info({code:code,info:info,msg:msg});
};
helper.warn = function(code,info,msg){
    if(msg == null)
        msg = "";
    logWarn.warn({code:code,info:info,msg:msg});
    logConsole.warn({code:code,info:info,msg:msg});
};
helper.error = function(code,info,msg, exp){
    ...
    if(exp != null)
        msg += "\r\n" + exp;
    logErr.error({code:code,info:info,msg:msg});
    logConsole.error({code:code,info:info,msg:msg});
};
// 配合express用的方法
