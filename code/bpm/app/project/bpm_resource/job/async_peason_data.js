...
function savePeason(result,type){
    ...
    for(let i in result){
        ...
        if( result[i].phone==null || result[i].phone==""){
            //电话为空  机构为空 不导入  导出到文件
            writeFile("e:\\file_no_tel"+a+".txt",JSON.stringify(result[i]))
            continue;
        }

        if(result[i].orgId==null ||  result[i].orgId==""){
            //电话为空  机构为空 不导入  导出到文件
            writeFile("e:\\file_no_org_"+a+".txt",JSON.stringify(result[i]))
            continue;
        }
        //查询此人员是否已在工单系统中
        model_org.$User.find({"login_account": result[i].phone},function(err,resp) {
                    ...
                    if(resp.length>0){//已存在，做何处理
                        if (res) {
                            if (res.length > 0) {//查到orgid
                                ...
                                var conditions = {login_account:inst.login_account};
                                var update = {$set: inst};
                        ...
                        if (res) {
                            if(res.length>0) {//查到orgid
                                ...
                                inst.__v = 0;
                                // 实例模型，调用保存方法
                                model_org.$User(inst).save(function(error){
                                    if(error) {
                                        console.log(inst);
