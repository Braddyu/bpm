    ...
    return new Promise(function (resolve, reject) {
        ...
        model_user.$User.find({"user_no": userCode}, function (err, res) {
            if (err) {
                ...
                //待处理任务
                let wait_handle_match = [{
                    proc_inst_task_sign: 1,
                    $or: [{proc_inst_task_assignee: userCode}, {proc_inst_task_work_id: work_id}]
                }];
                //待认领任务
                let wait_claim_match = [
                    {
                        proc_inst_task_sign: 0,
                        org_length: {$eq: 0},
                        proc_inst_task_user_role: {$in: user_roles}
                    },
                    {
                        proc_inst_task_sign: 0,
                        role_length: {$eq: 0},
                        proc_inst_task_user_org: {$in: user_org}
                    },
                    {
                        proc_inst_task_sign: 0,
                        role_length: {$gt: 0},
...
exports.getMyTaskQuery = function (taskId, user_no) {
    return new Promise(function (resolve, reject) {
        model.$ProcessInstTask.find({
            ...
            'proc_inst_task_assignee': user_no
        }, function (e, r) {
            if (e) {
...
exports.getMyTaskList = function (userCode, paramMap, joinup_sys) {
    return new Promise(function (resolve, reject) {
        ...
        var query = model.$ProcessInstTask.find({
            $and: [{
                'proc_inst_task_status': 0,
                'joinup_sys': joinup_sys,
                'proc_inst_task_assignee': {'$in': userArr}
            }]
        });
...
exports.getMyCompleteTaskList = function (userCode, roleArr, orgArr) {
    ...
    var p = new Promise(function (resolve, reject) {
        ...
        userArr.push(userCode);
        var query = model.$ProcessInstTask.find({
            "$or": [{'proc_inst_task_assignee': {"$in": userArr}}, {
                'proc_inst_task_user_role': {"$in": roleArr},
                'proc_inst_task_user_org': {"$in": orgArr}
    ...
    var p = new Promise(function (resolve, reject) {
        async function find_insId() {
            ...
            if (rs.length > 0) {
                ...
                let res = await model.$ProcessTaskHistroy.find({
                    'proc_inst_task_assignee': user_no,
                    'joinup_sys': joinup_sys
                });
                if (res.length > 0) {
                ...
                let result = await model.$ProcessTaskHistroy.find({
                    'proc_inst_task_assignee': user_no,
                    'joinup_sys': joinup_sys
                });
                if (result.length > 0) {
