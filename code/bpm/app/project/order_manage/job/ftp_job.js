...
function ftp() {
    ...
    ftp_util.downloadFileList(config.local_haunghe_path, config.ftp_huanghe_get, function (err, res) {
        if (err) {
            ...
            readDirSync(config.local_haunghe_path)
            ftp_util.end();
        }
    });
...
async function readDirSync(path){
    ...
    for(let ele of pa){
        ...
        if(!info.isDirectory()){
            ...
            if (ele.indexOf("txt") >=0 && ele.indexOf(yesterday) >= 0) {
                await  readFile(ele)
            }
        }
    }
    ...
    return new Promise(function(resolve,reject){
         try{
             ...
             objReadline.on('close', ()=>{
                 console.log(fileName+"开始插入："+arr.length);
                 mistake_model.$ProcessMistake.create(arr,  function (error, rs) {

                     if (error) {

                     } else {
                         console.log(fileName+"数据插入到数据库成功", index);
                         var arr1 = [];
                         //将插入成功的文件记录到日志表中
                         ...
                         mistake_model.$ProcessFtpMistakeReadlogs.create(arr1, function (err, re) {
                             if (err) {
                                 ...
                                 console.log("oldFilrPath:",oldFilrPath,";newFilrPath:",newFilrPath);
