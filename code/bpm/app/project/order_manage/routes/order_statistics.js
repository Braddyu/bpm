...
router.route('/export_excel_detail').get(function(req,res){
    ...
    var startDate = req.query.startDate;//开始插入时间
    var endDate = req.query.endDate;//结束插入时间
    var proc_inst_task_type = req.query.proc_inst_task_type;//工单状态is


    var  istodo=req.query.todo;//是否为网格经理导出
    ...
    var org_id;
    var level;
    console.log("todo",istodo,proc_inst_task_type);
    if(istodo=='1'){
        if(user_no=='admin'){
            // level='2';
    ...
    console.log("params",org_id,proc_code,level,status,dispatch_time,startDate,endDate);
    // 调用分页
    service.exportDetailList(org_id,proc_code,level,status,dispatch_time,startDate,endDate,proc_inst_task_type)
...
router.route('/channel_detail_list').post(function(req,res){
    ...
    console.log("params",status,proc_code,dispatch_time,startDate,endDate,page,size,proc_inst_task_type);
    console.log(org_id,level);
    if(org_id && level){
        // 调用分页
        service.detail_list(page,size,org_id,level,status,proc_code,dispatch_time,startDate,endDate,proc_inst_task_type)
            .then(function(result){
                utils.respJsonData(res, result);
            })
            .catch(function(err){
