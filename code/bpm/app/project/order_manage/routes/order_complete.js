...
router.route('/order_customer_reason').post(function (req, res) {
    ...
    completeService.getMyCompleteTaskQueryCustomer(page, rows, startDate, endDate, work_order_number, task_type).then(function (taskresult) {
        utils.respJsonData(res, taskresult);
    }).catch(function (err_inst) {
        console.log("route-getMyTaskList", "获取我的已办数据异常", err_inst);
        utils.respMsg(res, false, '1000', '获取数据异常', null, err_inst);
...
router.route('/acceptBatch').post(function (req, res) {
    ...
    let inst_id = req.body.inst_id;//实例id
    let current_user = req.session.current_user;
    completeService.acceptBatch(inst_id, current_user.user_name, current_user.user_no).then(function (taskresult) {
        utils.respJsonData(res, taskresult);
    })
